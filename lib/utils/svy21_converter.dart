...
class Svy21Converter {
  ...
  static LatLng toLatLng({required double northing, required double easting}) {
    ...
    final double vPrime = _a / math.sqrt(1 - e2 * sinLatPrime * sinLatPrime);
    ...
    final double latTerm2 =
        latFactor *
        ((eastingPrime * x3) / 24) *
        ((-4 * psiPrime * psiPrime) +
            (9 * psiPrime * (1 - tPrime * tPrime)) +
            (12 * tPrime * tPrime));
    final double latTerm3 =
        latFactor *
        ((eastingPrime * x5) / 720) *
    ...
    return LatLng(latPrime * 180 / math.pi, lon * 180 / math.pi);
  }

  static double _calcM(double lat, double e2, double e4, double e6) {
    return _a *
